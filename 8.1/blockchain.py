...
# consignee：收件人
# memo:备注信息
def make_a_block(index,timestamp,consignor,consignee,memo,previous_hash):
    block={}
    block["index"]=index
    ...
    block["hash"]=hash(index,consignor,consignee,memo,timestamp,previous_hash)
    return block

def add_a_block(consignor,consignee,memo):
    last_block = blockchain[len(blockchain)-1]
    index=last_block["index"]+1
    timestamp=int(round(time() * 1000))
    previous_hash=last_block["hash"]
    blockchain.append(make_a_block(index,timestamp,consignor,consignee,memo,previous_hash))
